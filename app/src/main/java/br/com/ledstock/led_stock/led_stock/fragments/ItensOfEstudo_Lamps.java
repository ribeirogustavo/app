...
public class ItensOfEstudo_Lamps extends android.support.v4.app.Fragment {
    ...
    public static List<String> Lamps;

    public ItensOfEstudo_Lamps() {
        // Required empty public constructor
    }

    public static ItensOfEstudo_Lamps newInstance() {
        ItensOfEstudo_Lamps fragment = new ItensOfEstudo_Lamps();
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        getActivity().registerReceiver(RefreshItensOfEstudoLamps, new IntentFilter("REFRESH_ITENS_LAMPS"));
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_itens_of_estudo_fragment, container, false);
        ...
        if (!id_ambiente_estudo_remote.equals("0")) {
            c = db.SelectLampsOfAmbienteOfEstudo(String.valueOf(ID_AMBIENTE_OF_ESTUDO), id_ambiente_estudo_remote);
        } else {
            c = db.SelectLampsOfAmbienteOfEstudo(String.valueOf(ID_AMBIENTE_OF_ESTUDO), "");
        }
        ...
        return new ItensOfEstudoLampLedAdapter.LampsOnClickListener() {
            ...
            public void onClickLamps(View view, int idx) {
                ...
                if (actionMode != null) {
                    ...
                    int index = Lamps.indexOf(String.valueOf(idx));
                    if (index == -1) {
                        Lamps.add(String.valueOf(idx));
                    } else {
                        Lamps.remove(index);
                    }
    ...
    private void updateActionModetitle() {
        if (actionMode != null) {
            actionMode.setTitle("Selecione as Lâmpadas !");
            actionMode.setSubtitle(null);

            int lenght = Lamps.size();

            if (lenght == 0) {
                actionMode.finish();
                Lamps.clear();
            } else if (lenght == 1) {
                actionMode.setSubtitle("1 Lâmpada selecionada !");
            } else if (lenght > 1) {
                actionMode.setSubtitle(lenght + " Lâmpadas selecionadas !");
            }
    ...
    private BroadcastReceiver RefreshItensOfEstudoLamps = new BroadcastReceiver() {
        @Override
    ...
    public void onDestroy() {
        ...
        getActivity().unregisterReceiver(RefreshItensOfEstudoLamps);
    }
                ...
                if (item.getItemId() == R.id.action_delete) {
                    ...
                    try {
                        for (int i = 0; i < Lamps.size(); i++) {
                            db.DeleteItensOfEstudo(Lamps.get(i));
                            LedService remote = new LedService();
                            remote.DeleteItensOfEstudoRemote(Lamps.get(i));
                        }
                    }finally {
                        db.close();
                        Lamps.clear();
                        ...
                        getActivity().sendBroadcast(intent);
                        if (size == 1){
                            Toast.makeText(getActivity(), "Lâmpada Excluida com Sucesso !", Toast.LENGTH_SHORT).show();
                        }else if (size > 1){
                            Toast.makeText(getActivity(), "Lâmpadas Excluidas com Sucesso !", Toast.LENGTH_SHORT).show();
                        }
                    }
